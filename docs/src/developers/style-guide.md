...
!!! info
    The style guide is always a work in progress, and not all DNNSP2PT code
    follows the rules. When modifying DNNSP2PT, please fix the style violations
...
DNNSP2PT uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/DNNSP2PT.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the DNNSP2PT directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to DNNSP2PT have
